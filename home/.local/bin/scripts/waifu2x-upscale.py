...
waifu2x_model = r"/usr/share/waifu2x-ncnn-vulkan/models-cunet"
...
denoise = (
    int(os.getenv("UPSCALE_DENOISE") or 0)
    if os.getenv("UPSCALE_DENOISE") is not None
    else None
)
...
if scale > 0 and (tx > 0 or ty > 0 or mx > 0 or my > 0):
    raise Exception("Cannot specify scaling factor alongside widths or heights.")
